...
func solve(_ tables: String, _ people: String) {
    var pq0 = PriorityQueue<Int>(by: <)
    var pq1 = PriorityQueue<Int>(by: <)
    for (i, t) in tables.enumerated() {
        if t == "0" { pq0.enqueue(i) }
        else if t == "1" { pq1.enqueue(i) }
    }
